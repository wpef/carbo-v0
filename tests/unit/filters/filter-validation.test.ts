// Unit tests for filter-validation.ts (port v4 → v5, sémantique inchangée)
// Realistic Salesforce Contact field data (Principle IV)
//
// v5 : validateFilter attend { apiName, dataType }[] — les propriétés
// supplémentaires des fixtures v4 (label, isRequired…) sont tolérées/inutiles.

import { describe, it, expect } from 'vitest'
import { validateFilter } from '@/features/filters/lib/filter-validation'

interface SourceFieldFixture {
  apiName: string
  label: string
  dataType: string
  isRequired: boolean
...
const SF_CONTACT_FIELDS: SourceFieldFixture[] = [
