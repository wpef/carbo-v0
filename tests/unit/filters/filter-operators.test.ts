// Unit tests for filter-operators.ts (port v4 → v5, sémantique inchangée)
// 02-domain-rules règle 5 — 11 opérateurs, labels français
...
describe('FILTER_OPERATORS metadata', () => {
  ...
  it('all operators have valid values from the accepted list', () => {
    ...
    const expected = new Set<string>(ALL_OPERATOR_VALUES)
...
describe('isValidOperator', () => {
  ...
  it('returns false for unknown operators', () => {
    ...
    expect(isValidOperator('NOT_IN')).toBe(false) // not in schema
    expect(isValidOperator('IN')).toBe(false) // not in schema
  })
