// Tests unitaires — record-preview-service (aperçu paginé + sanitisation binaire) — port v4 → v5.
// Couvre : pagination adaptateur, sanitisation binaire (Buffer/Uint8Array → "[binary data]"),
// audit RECORDS_PREVIEWED, propagation d'erreurs.
//
// Changements v4 → v5 :
//  - DB : @/lib/prisma (prisma.*) → @/lib/db (db.*).
//  - registre : @/lib/adapters/registry → @/features/connectors/registry.
//  - service : @/features/schema/services/record-preview-service
//              → @/features/schema/record-preview-service.
//  - la connexion doit avoir status "CONNECTED" (v5 exige explicitement CONNECTED).
//  - erreurs : les 3 classes v4 (PlanNotFound / ConnectionNotFound / NotConnected)
//    sont fusionnées en une seule RecordPreviewUnavailableError en v5.
//    On teste que ça lève RecordPreviewUnavailableError avec le message français adéquat.
// Principe IV : formes d'enregistrements CRM réalistes.
...
const MOCK_PLAN = {
  id: PLAN_ID,
  sourceConnection: { id: CONN_ID, name: "Salesforce ACME", adapterType: "salesforce", status: "CONNECTED" },
...
function mockFindUnique() {
  return vi.mocked(db.migrationPlan.findUnique);
}
...
describe("fetchRecordPage", () => {
  ...
  it('replaces Buffer values with "[binary data]" placeholder', async () => {
    const recordsWithBinary = [{ ...SF_CONTACT_RECORDS[0], Photo: Buffer.from("binary-data") }];
    const mockAdapter = {
  ...
  it('replaces Uint8Array values with "[binary data]" placeholder', async () => {
    const recordsWithUint8 = [{ ...SF_CONTACT_RECORDS[0], Thumbnail: new Uint8Array([1, 2, 3]) }];
    const mockAdapter = {
  ...
  // v5 : PlanNotFound → RecordPreviewUnavailableError (message "Plan introuvable").
  it("throws RecordPreviewUnavailableError when plan does not exist", async () => {
    mockFindUnique().mockResolvedValue(null as never);
    await expect(fetchRecordPage(PLAN_ID, "SOURCE", "Contact", 1, 50)).rejects.toBeInstanceOf(
      RecordPreviewUnavailableError,
    );
    mockFindUnique().mockResolvedValue(null as never);
    await expect(fetchRecordPage(PLAN_ID, "SOURCE", "Contact", 1, 50)).rejects.toThrow(/Plan introuvable/);
  });

  // v5 : ConnectionNotFound → RecordPreviewUnavailableError (message "Aucune connexion source").
  it("throws RecordPreviewUnavailableError when no source connection", async () => {
    mockFindUnique().mockResolvedValue({ ...MOCK_PLAN, sourceConnection: null } as never);
    await expect(fetchRecordPage(PLAN_ID, "SOURCE", "Contact", 1, 50)).rejects.toBeInstanceOf(
      RecordPreviewUnavailableError,
    );
    mockFindUnique().mockResolvedValue({ ...MOCK_PLAN, sourceConnection: null } as never);
    await expect(fetchRecordPage(PLAN_ID, "SOURCE", "Contact", 1, 50)).rejects.toThrow(
      /Aucune connexion source/,
    );
  });

  // v5 : NotConnected → RecordPreviewUnavailableError (message "n'est pas active").
  it("throws RecordPreviewUnavailableError when connection is EXPIRED", async () => {
    ...
    await expect(fetchRecordPage(PLAN_ID, "SOURCE", "Contact", 1, 50)).rejects.toThrow(/n'est pas active/);
  });
