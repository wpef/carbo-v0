...
- **FR-017** (Step navigation — high-water mark): Every step up to the furthest the consultant has reached MUST be clickable in the sidebar to navigate (back **and** forward among validated steps); steps beyond the furthest reached remain locked. "Furthest reached" = `max(recorded currentStep, step currently open)`. Reaching a step ahead of `currentStep` (via a "Continue" button **or** a direct URL) MUST persist the high-water mark (forward-only `PATCH /step`), so navigating back to an earlier step never re-locks the steps already validated. The guided "Continue" buttons at each step boundary MUST advance `currentStep` (intra-SOURCE sub-steps source→objects→fields do not; the cross-step transitions source/fields→DESTINATION, destination/fields→OBJECT_MAPPING, object-mapping→FIELD_MAPPING, field-mapping→DOCUMENTS each do) so the plan reliably reaches DOCUMENTS and becomes READY. <!-- Added: 2026-06-17 — fixes the recette regression "cannot click back to validated steps"; advancement was previously dropped by the rich sub-pages, so the plan never reached READY. -->
