...
3. **Given** the object selection list, **When** the consultant expands an object, **Then** the system fetches on-demand and displays the record count and a sample of fields (first 5-10 fields) for that object. This call is only made when the consultant explicitly clicks to expand.
...
- **FR-005**: The system MUST allow on-demand expansion of any object to display its record count and sample fields. The API call MUST only be triggered when the consultant explicitly requests it.
...
- **SC-003**: On-demand expand (record count + sample fields) completes in under 10 seconds.
...
- The connector adapter provides a method to retrieve a sample of fields for a single object on demand.
