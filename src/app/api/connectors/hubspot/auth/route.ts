...
export async function GET(req: NextRequest) {
  ...
  await logAction(planId, 'HS_OAUTH_INIT', { callbackUrl: config.callbackUrl })
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    await logAction(planId, 'HS_PRIVATE_APP_INVALID', { message })
  ...
  try {
    const connection = await upsertDestinationConnectionRaw(
      planId,
      'hubspot',
      connectionConfig as unknown as Record<string, unknown>,
      'CONNECTED',
    )
    await logAction(planId, 'HS_PRIVATE_APP_CONNECTED', { portalId: portal.portalId })
    return NextResponse.json(
      {
        id: connection.id,
