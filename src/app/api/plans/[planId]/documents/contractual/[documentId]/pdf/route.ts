 ...
 *
 * Generate and return the contractual document as a downloadable PDF.
...
export async function GET(_req: NextRequest, { params }: RouteParams) {
  ...
  try {
    const result = await generatePdf(doc.html, {
      title: `${doc.planName} — Contractual Document (${doc.referenceNumber})`,
      date: new Date(doc.generatedAt).toLocaleDateString('fr-FR'),
      pageSize: 'A4',
    })

    const dateStr = new Date(doc.generatedAt).toISOString().slice(0, 10)
    const filename = sanitizeFilename(doc.planName, 'contractual-document', dateStr)

    await logAction(planId, 'pdf_export_contractual_document', {
      documentId,
    ...
    console.error('[PDF] Contractual document PDF generation failed:', message)
