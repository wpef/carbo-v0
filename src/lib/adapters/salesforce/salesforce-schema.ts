// Salesforce — mapping jsforce describe responses to ConnectorObject / ConnectorField (v4 port)
 ...
 * Salesforce doesn't expose a direct flag at describe level; we use `lookup` for
 * all standard reference fields. The mapping UI can refine if needed.
 */
function inferRelationshipType(field: DescribeField): 'lookup' | 'master-detail' | 'external' | undefined {
...
export function mapDescribeToFields(result: DescribeResult): ConnectorField[] {
  return result.fields.map<ConnectorField>((f) => ({
    ...
    isAccessible: true, // FLS-restricted fields would show false; jsforce omits them from describe
...
/**
 * Normalise SF field types to the Carbo-v0 type vocabulary.
