// Salesforce — SOQL query building + field stats (v4 port)
// Ref: specs/adapters/salesforce/

import type { ConnectorRecord, FieldStats, PaginatedRecords } from '@/lib/types/connector'
 ...
 * by the demo adapters and the API route (which requires page >= 1).
 *
 * Salesforce caps OFFSET at 2000; we raise an error if exceeded.
 */
export function buildSoqlQuery(
  ...
  const size = Math.max(1, Math.min(200, Math.floor(pageSize))) // cap preview columns
  const offset = (pageNum - 1) * size

  if (offset > 2000) {
    throw new Error(`SALESFORCE_OFFSET_EXCEEDED: OFFSET ${offset} exceeds Salesforce limit of 2000.`)
  }
 ...
 * the consumed window does not yet cover the total.
 */
 ...
 * - sampleValues: up to 5 distinct non-null values (insertion order).
 */
