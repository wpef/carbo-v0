// HubSpot — Search API (paginated) + field stats (v4 port)
...
/**
 * Cursor store keyed by `${scope}::${objectType}::${page}` so we can paginate
 ...
 *
 * `page` is **1-indexed** (page=1 → first page, no cursor walk required).
 */
...
/** Calculate per-field statistics from a set of records. */
