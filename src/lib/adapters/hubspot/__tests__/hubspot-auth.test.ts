// Unit tests for hubspot-auth.ts
// No real network calls — fetch is mocked via vi.stubGlobal.
...
import { HS_AUTHORIZE_URL, HS_TOKEN_URL } from '../hubspot-constants'
...
describe('validatePrivateAppToken', () => {
  ...
  it('returns portal info on success', async () => {
    ...
    const info = await validatePrivateAppToken('valid-token')
  ...
  it('sends the token as a Bearer Authorization header', async () => {
    ...
    await validatePrivateAppToken('my-token')
  ...
  it('throws HubSpotAuthError on 401', async () => {
    ...
    await expect(validatePrivateAppToken('bad-token')).rejects.toThrow(HubSpotAuthError)
  })
  ...
  it('throws HubSpotAuthError when portalId is absent from response', async () => {
    ...
    await expect(validatePrivateAppToken('token')).rejects.toThrow(HubSpotAuthError)
  })
})
