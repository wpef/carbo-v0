...
// Two-column header (source field / dest field) + type-specific section (D1-D4) + footer buttons.
// Ported from v3 src/components/mapping/MigrationLogicModal.tsx
...
interface MigrationLogicModalProps {
  ...
  /** D4 message (null for D1/D2/D3) */
  informationalMessage: string | null
  /** Existing migration logic loaded from server (null = not yet saved) */
  migrationLogic: MigrationLogicDTO | null
  /** D1: picklist values from source + destination fields */
  sourcePicklistValues: string[]
  destPicklistValues: string[]
  /** D2: sample source record values */
  sampleSourceValues: string[]
  loading: boolean
  saving: boolean
  error: string
  /** Save callback → status DEFINED (orange) */
  onSave: (input: {
    sectionType: SectionType
    valueEquivalences?: Array<{ sourceValue: string; destinationValue: string }>
    promptText?: string
  }) => Promise<{ error?: string }>
  /** Validate callback → status VALIDATED (green) */
  onValidate: (input: {
    sectionType: SectionType
    valueEquivalences?: Array<{ sourceValue: string; destinationValue: string }>
    promptText?: string
  }) => Promise<{ error?: string }>
  /** Classify samples for D2 preview */
  onClassify: (
    promptText: string,
...
export function MigrationLogicModal({
  ...
  const handleSave = useCallback(async () => {
    ...
    if (result.error) { setActionError(result.error) } else { onClose() }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [sectionType, currentEquivalences, currentPromptText, onSave, onClose])

  const handleValidate = useCallback(async () => {
    setActionError('')
    const result = await onValidate(buildInput())
    if (result.error) { setActionError(result.error) } else { onClose() }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [sectionType, currentEquivalences, currentPromptText, onValidate, onClose])
  ...
  const typeLabel = (type: string) => {
    const map: Record<string, string> = {
      ...
      picklist: 'Picklist', boolean: 'Checkbox', multipicklist: 'Multi-Picklist',
    }
  ...
  return (
    <Dialog open={open} onOpenChange={(isOpen) => { if (!isOpen) onClose() }}>
      <DialogContent className="max-w-2xl w-full" showCloseButton={false}>
        <DialogHeader>
        ...
        <div className="py-2 min-h-[200px]">
          {loading ? (
            ...
            <InformationalCopySection message={informationalMessage ?? 'La valeur sera copiée.'} />
          ) : null}
        </div>
        ...
        <DialogFooter>
          ...
          {/* Validate disabled for D3 */}
          <Button onClick={handleValidate} disabled={saving || isError}>
