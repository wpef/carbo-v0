// 004-source-object-selection — Expanded panel: record count, fields, sample records on-demand
...
export function ObjectExpandPanel({ planId, objectApiName }: ObjectExpandPanelProps) {
  ...
  useEffect(() => {
    setLoading(true)
    setError('')
    fetch(`/api/plans/${planId}/source/objects/${encodeURIComponent(objectApiName)}/expand`)
      .then(async (res) => {
        if (!res.ok) {
          const json = await res.json()
          throw new Error(json.message ?? json.error ?? 'Failed to load object details.')
        }
    ...
    <div className="bg-muted/20 px-6 py-4 space-y-6 border-b">
      ...
      <div className="flex items-center gap-4 text-sm">
        <span className="text-muted-foreground">
          <span className="font-medium text-foreground">{data.recordCount.toLocaleString()}</span> records
        </span>
        <span className="text-muted-foreground">
                ...
                <tr key={field.apiName} className={idx % 2 === 0 ? 'bg-background' : 'bg-muted/20'}>
                  ...
                  <td className="px-3 py-1.5">
                    <span className="flex gap-1 flex-wrap">
                      {field.isRequired && (
                        <span className="text-[10px] bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400 rounded px-1">
                          req
                        </span>
                      )}
                      {field.isUnique && (
                        <span className="text-[10px] bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400 rounded px-1">
                          unique
                        </span>
                      )}
                      {field.isReadOnly && (
