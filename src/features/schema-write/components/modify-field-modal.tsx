// 022-schema-write — Modify destination field modal (T015)
// Ported from v3 src/components/schema-write/modify-field-modal.tsx
// Uses dialog.tsx from the v4 component library.
...
export interface ModifyableField {
  apiName: string
  label: string
  dataType: string
  description?: string
...
interface ModifyFieldModalProps {
  open: boolean
  onClose: () => void
  field: ModifyableField
  /** Types supported by the destination adapter — when provided, replaces the default list */
...
export function ModifyFieldModal({
  ...
  const [type, setType] = useState(field.dataType)
  const [description, setDescription] = useState(field.description ?? '')
  const [picklistValues, setPicklistValues] = useState(field.picklistValues?.join('\n') ?? '')
  ...
  const typeOptions =
    supportedFieldTypes && supportedFieldTypes.length > 0
      ? supportedFieldTypes.map((v) => {
          const found = FIELD_DATA_TYPES.find((t) => t.value === v)
          return { value: v, label: found?.label ?? v }
        })
      : FIELD_DATA_TYPES

  // Sync form state when the target field changes
  useEffect(() => {
    setLabel(field.label)
    setType(field.dataType)
    ...
    <Dialog open={open} onOpenChange={(isOpen) => { if (!isOpen) onClose() }}>
      <DialogContent className="sm:max-w-lg" showCloseButton>
        ...
        <div className="space-y-4 py-2">
          ...
          {/* Type */}
          <div className="space-y-1">
            <label htmlFor="modify-field-type" className="text-sm font-medium">
              Type de données
            </label>
            <select
              id="modify-field-type"
              value={type}
